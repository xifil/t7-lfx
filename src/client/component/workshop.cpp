...
namespace workshop {
	namespace {
		...
		void load_workshop_data(game::workshop_data& item) {
			...
			if (!doc.HasMember("Title") || !doc.HasMember("Description") || !doc.HasMember("FolderName") || !doc.HasMember("PublisherID")) {
	...
	bool check_valid_usermap_id(const std::string& mapname, const std::string& pub_id, const std::string& workshop_id) {
		if (!game::DB_FileExists(mapname.data(), 0) && pub_id.empty()) {
			...
			if (utils::string::is_numeric(mapname.data())) {
				...
				game::UI_OpenErrorPopupWithMessage(0, 0x100, utils::string::va("Could not download this mod folder name is not numeric and"
