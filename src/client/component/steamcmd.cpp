...
namespace steamcmd {
    int start_new_process(const char* exePath, bool Hide_Window, bool waittill_done, const char* arguments) {
		...
		if (CreateProcess(NULL, (LPSTR)commandLine.c_str(), NULL, NULL, FALSE, 0, NULL, NULL, &startupInfo, &pi)) {
			if (waittill_done) {
	...
	int download_workshop_item(std::string workshop_id, std::string modtype) {
		...
		while (workshop::downloading_workshop_item && tries < max_tries) {
			...
			int result = start_new_process("./steamcmd/steamcmd.exe", false, true, ("+login anonymous app_update 311210 +workshop_download_item 311210 "
