...
#			endif
	...
	struct component final : client_component {
		...
		void post_unpack() override {
			const auto res = start_mod("\xE2\x98\x84\xEF\xB8\x8F" " t7-lfx"s, steam::SteamUtils()->GetAppID());
	...
	void update_map_client(subscribed_item_map& map) {
		...
		for (uint32_t i = 0; i < result; ++i) {
			...
			item.available = client_ugc.invoke<bool>("GetItemInstallInfo", app_id, ids[i], &item.size_on_disk, buffer, sizeof(buffer), &item.time_stamp);
			item.path = buffer;
	...
	void update_map_steam(subscribed_item_map& map) {
		...
		for (uint32_t i = 0; i < result; ++i) {
			...
			item.available = steam_ugc->GetItemInstallInfo(ids[i], &item.size_on_disk, buffer, sizeof(buffer), &item.time_stamp);
			item.path = buffer;
