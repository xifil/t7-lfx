...
namespace exception {
	namespace {
		...
		void display_error_dialog() {
			...
			for (const auto& [known_exception, known_msg] : known_errors) {
				if (known_exception.first == exception_data.code && known_exception.second == address) {
					error_str += "\n";
					error_str += "This is a known issue; ";
					error_str += known_msg;
					error_str += "\n";
					break;
				}
