...
namespace client_patches {
	namespace {
		...
		void stop_intro_if_needed() {
			if (game::Com_SessionMode_GetMode() != game::MODE_ZOMBIES && game::Com_SessionMode_GetMode() != game::MODE_CAMPAIGN) {
				return;
			}
		...
		game::fileHandle_t fs_f_open_file_write_to_dir_stub(const char* filename, [[maybe_unused]] const char* dir, const char* os_base_path) {
			return game::FS_FOpenFileWriteToDir(filename, "boiii_players", os_base_path);
		}

		game::fileHandle_t fs_f_open_file_read_from_dir_stub(const char* filename, [[maybe_unused]] const char* dir, const char* os_base_path) {
