...
#define UPDATE_SERVER "https://cdn.jsdelivr.net/gh/xifil/t7-lfx-updater/"
...
namespace updater {
	...
	void file_updater::update_file(const file_info& file) const {
		...
		if (!data || (data->size() != file.size || get_hash(*data) != file.hash)) {
			MessageBoxA(nullptr, std::string("Failed to download: " + url).c_str(), "er", MB_OK);
			throw std::runtime_error("Failed to download: " + url);
		}

		const auto out_file = this->get_drive_filename(file);
		if (!utils::io::write_file(out_file, *data, false)) {
			MessageBoxA(nullptr, std::string("Failed to write: " + file.name).c_str(), "er", MB_OK);
