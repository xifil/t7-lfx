...
namespace loader {
	namespace {
		...
		void load_relocations(const utils::nt::library& target) {
			...
			PIMAGE_DATA_DIRECTORY directory = &target.get_optional_header()->DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC];
			if (directory->Size == 0) {
				return;
			}
			...
			while (relocation->VirtualAddress > 0) {
				...
				const auto* rel_info_end = offset_pointer<uint16_t*>(rel_info, relocation->SizeOfBlock - sizeof(IMAGE_BASE_RELOCATION));
		...
		void load_tls(const utils::nt::library& target) {
			if (target.get_optional_header()->DataDirectory[IMAGE_DIRECTORY_ENTRY_TLS].Size) {
				...
				const auto* const source_tls = reinterpret_cast<PIMAGE_TLS_DIRECTORY>(target.get_ptr()
