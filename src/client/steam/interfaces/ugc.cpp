...
namespace steam {
	uint64_t ugc::CreateQueryUserUGCRequest(uint_least32_t unAccountID, uint32_t eListType, uint32_t eMatchingUGCType, uint32_t eSortOrder, uint32_t nCreatorAppID,
		uint32_t nConsumerAppID, uint32_t unPage)
	{
		return 0;
	}

	uint64_t ugc::CreateQueryAllUGCRequest(uint32_t eQueryType, uint32_t eMatchingeMatchingUGCTypeFileType, uint32_t nCreatorAppID, uint32_t nConsumerAppID,
		uint32_t unPage)
	{
		return 0;
	}

	uint64_t ugc::CreateQueryUGCDetailsRequest(uint64_t* pvecPublishedFileID, uint32_t unNumPublishedFileIDs) {
		return 0;
	}
	...
	bool ugc::GetQueryUGCMetadata(uint64_t handle, uint32_t index, char* pchMetadata, uint32_t cchMetadatasize) {
		return false;
	}

	bool ugc::GetQueryUGCChildren(uint64_t handle, uint32_t index, uint64_t* pvecPublishedFileID, uint32_t cMaxEntries) {
		return false;
	}

	bool ugc::GetQueryUGCStatistic(uint64_t handle, uint32_t index, uint32_t eStatType, uint32_t* pStatValue) {
		return false;
	}
	...
	bool ugc::GetQueryUGCAdditionalPreview(uint64_t handle, uint32_t index, uint32_t previewIndex, char* pchURLOrVideoID, uint32_t cchURLSize,
		char* pchOriginalFileName, uint32_t cchOriginalFileNameSize, void* pPreviewType)
	{
		return false;
	}
	...
	bool ugc::GetQueryUGCKeyValueTag(uint64_t handle, uint32_t index, uint32_t keyValueTagIndex, char* pchKey, uint32_t cchKeySize, char* pchValue,
	...
	uint32_t ugc::GetItemUpdateProgress(uint64_t handle, uint64_t* punBytesProcessed, uint64_t* punBytesTotal) {
		return 0;
	}
	...
	bool ugc::GetItemInstallInfo(uint64_t nPublishedFileID, uint64_t* punSizeOnDisk, char* pchFolder, uint32_t cchFolderSize, uint32_t* punTimeStamp) {
		bool found = false;
		::steam_proxy::access_subscribed_items([&](const steam_proxy::subscribed_item_map& items) {
			const auto entry = items.find(nPublishedFileID);
			if (entry != items.end()) {
				...
				memcpy(pchFolder, item.path.data(), std::min(item.path.size() + 1, static_cast<size_t>(cchFolderSize)));
	...
	bool ugc::GetItemDownloadInfo(uint64_t nPublishedFileID, uint64_t* punBytesDownloaded, uint64_t* punBytesTotal) {
		return false;
	}
	...
	uint64_t ugc::StartPlaytimeTracking(uint64_t* pvecPublishedFileID, uint32_t unNumPublishedFileIDs) {
		return 0;
	}

	uint64_t ugc::StopPlaytimeTracking(uint64_t* pvecPublishedFileID, uint32_t unNumPublishedFileIDs) {
		return 0;
	}
