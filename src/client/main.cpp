...

	{
		...
		try {
			...
			if (!has_client && !has_server) {
				throw std::runtime_error("Can't find a valid BlackOps3.exe or BlackOps3_UnrankedDedicatedServer.exe. Make sure you put boiii.exe in your"
					" Black Ops 3 installation folder.");
			}
			...
			if (!is_server && !game::is_client()) {
				if (game::is_legacy_client()) {
					throw std::runtime_error("You are using the outdated BlackOps3.exe. This version is not supported anymore."
						" Please use the latest binary from Steam.");
				}
